		...
		bool isProtected = false)
	{
		...
		foreach (Event @event in @class.AllEvents().Where(predicate))
		{
			...
			sb.Append("\t\tpublic void ").Append(@event.Name).Append("(").Append(string.Join(", ",
				...
				.Append(")").AppendLine();
			sb.AppendLine("\t\t{");
			sb.Append("\t\t\tCastToMockOrThrow(mockRaises).Registrations.Raise(").Append(@event.GetUniqueNameString())
				.Append(", ")
		...
		if (@class.AllProperties().Any(predicate))
		{
			...
			foreach (Property property in @class.AllProperties().Where(predicate))
			{
				...
				sb.Append("\t\t\t\tvar propertySetup = new PropertySetup<").Append(property.Type.Fullname)
					.Append(">();").AppendLine();
				sb.AppendLine("\t\t\t\tCastToMockOrThrow(setup).Registrations.SetupProperty(")
		...
		bool isProtected = false)
	{
			         ...
			         .Select(x => x.IndexerParameters))
		{
			...
			sb.Append("\t\t\treturn CastToMockOrThrow(verify).GotIndexer(")
				.Append(string.Join(", ", indexerParameters.Value.Select((_, i) => $"parameter{i + 1}"))).Append(");")
				.AppendLine();
			sb.AppendLine("\t\t}");
		}
		...
		bool isProtected = false)
	{
		...
		foreach (Property indexer in @class.AllProperties().Where(predicate))
		{
			...
			sb.Append("\t\t\treturn CastToMockOrThrow(verify).SetIndexer(value, ")
				.Append(string.Join(", ", indexer.IndexerParameters.Value.Select((_, i) => $"parameter{i + 1}")))
				.Append(");").AppendLine();
			sb.AppendLine("\t\t}");
		}
