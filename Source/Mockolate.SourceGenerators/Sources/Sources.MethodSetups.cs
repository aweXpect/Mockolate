...
internal static partial class Sources
{
	...
	private static void AppendVoidMethodSetup(StringBuilder sb, int numberOfParameters)
	{
		...
		sb.Append("\t\tglobal::Mockolate.Setup.IVoidMethodSetupParallelCallbackBuilder<").Append(typeParams).Append("> ChangeScenario(string scenarioName);")
			.AppendLine();
		sb.AppendLine();
		...
		// ChangeScenario
		sb.Append("\t\t/// <inheritdoc cref=\"global::Mockolate.Setup.IVoidMethodSetup{").Append(typeParams).Append("}.ChangeScenario(string)\" />").AppendLine();
	...
	private static void AppendReturnMethodSetup(StringBuilder sb, int numberOfParameters)
	{
		...
		// ChangeScenario
		sb.Append("\t\t/// <inheritdoc cref=\"global::Mockolate.Setup.IReturnMethodSetup{TReturn, ").Append(typeParams).Append("}.ChangeScenario(string)\" />").AppendLine();
		...
		sb.AppendXmlSummary("Triggers any configured parameter callbacks for the method setup with the specified parameters.");
		sb.Append("\t\tpublic abstract void TriggerCallbacks(").Append(string.Join(", ", Enumerable.Range(1, numberOfParameters).Select(i => $"T{i} parameter{i}"))).Append(");").AppendLine();
		sb.AppendLine();
